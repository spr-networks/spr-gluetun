...
# matching `ARG <KEY>=` defaults / `# syntax=` line in the Dockerfile(s) plus the
# pinned gluetun image reference in docker-compose.yml. Read-only lookups
# (registry manifest inspect, go.dev). Review with `git diff`.
...
GLUETUN_TAG=qmcgaw/gluetun:v3.41.1
GO_MINOR=1.25
...
GLUETUN_REF="${GLUETUN_TAG}@$(mdigest "$GLUETUN_TAG")"
...
echo "Syncing pinned gluetun image in docker-compose.yml" >&2
replace_line docker-compose.yml '^    image: qmcgaw/gluetun:.*' "    image: ${GLUETUN_REF}"
